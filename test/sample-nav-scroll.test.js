 ...
 * start z2ui5_cl_smp_app_000, jump into a sample (tile press sends the
...
describe("smp_app_000 — jump-in and scroll restore", () => {
  jest.setTimeout(30000);

  const S = (o) => ({ ORIGIN: "http://x", PATHNAME: "/", SEARCH: "", HASH: "", ...o });

  test("tile press navigates, nav-back restores the scroll position", async () => {
    const draft = core("01/01/z2ui5_cl_ui5_srv_draft");
    const types = core("01/02/z2ui5_if_ui5_types");
    const mem = new Map();
    draft.set_store({ load: async (id) => mem.get(id), save: async (e) => void mem.set(e.id, e) });
    const handler = core("02/z2ui5_cl_ui5_http_handler");

    const start = await handler({ data: { value: { S_FRONT: S({ SEARCH: "?app_start=z2ui5_cl_smp_app_000" }) } } });
    expect(start.S_FRONT.APP).toBe("z2ui5_cl_smp_app_000");
  ...
  test("startup app links the samples gallery in What's next", async () => {
    ...
    expect(r.S_FRONT.APP).toBe("z2ui5_cl_ui5_app_start");
    // the What's-next button opens the getting-started gallery (upstream behavior)
    expect(r.S_FRONT.PARAMS.S_VIEW.XML).toContain("z2ui5_cl_smp_app_000");
  });
});
