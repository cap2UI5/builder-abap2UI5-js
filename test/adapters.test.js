...
describe("platform adapters", () => {
  ...
  gate("adapters/cap")("minimal CAP adapter serves statics, bootstrap and the roundtrip", async () => {
    const r = await bootAndRoundtrip("adapters/cap", 4411);
    expect(r).toEqual({
      indexOk: true,
      bootstrapOk: true,
      status: 200,
  ...
  gate("adapters/node")("bare node:http adapter serves statics, bootstrap and the roundtrip", async () => {
    const r = await bootAndRoundtrip("adapters/node", 4111);
    expect(r).toEqual({
      indexOk: true,
      bootstrapOk: true,
      status: 200,
  ...
  gate("adapters/express")("express adapter serves statics, bootstrap and the roundtrip", async () => {
    const r = await bootAndRoundtrip("adapters/express", 4211);
    expect(r).toEqual({
      indexOk: true,
      bootstrapOk: true,
      status: 200,
  ...
  gate("adapters/web")("web bundle builds and answers the roundtrip in-process", () => {
    const webDir = path.join(root, "adapters", "web");
