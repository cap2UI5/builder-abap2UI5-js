...
class z2ui5_cl_core_client {
  ...
  message_box_display() {
    ...
    let lv_title = ``;
    let lv_details = ``;
    if (!(z2ui5_cl_a2ui5_context.rtti_check_clike(text) === true || z2ui5_cl_a2ui5_context.rtti_check_clike(text) === `X`)) {
  ...
  nav_app_set_id({ app } = {}) {
    let result = ``;
    if (app != null) {
      throw new z2ui5_cx_a2ui5_error({ val: `NAV_APP_LEAVE_TO_INITIAL_APP_ERROR` });
    }
    if (!app.id_app) {
  ...
  nav_app_leave() {
    ...
    if (r_data !== undefined) {
      this.mo_action.ms_next.r_data = z2ui5_cl_a2ui5_context.conv_copy_ref_data(r_data);
    }
    result = this.nav_app_set_id({ app: app });
  }
  ...
  _bind_edit() {
    result = this.mo_srv_bind.main({ val: z2ui5_cl_a2ui5_context.conv_get_as_data_ref(val), type: z2ui5_if_core_types.cs_bind_type.two_way, config: { path_only: path, custom_filter: custom_filter, custom_filter_back: custom_filter_back, custom_mapper: custom_mapper, custom_mapper_back: custom_mapper_back, tab: z2ui5_cl_a2ui5_context.conv_get_as_data_ref(tab), tab_index: tab_index, switch_default_model: switch_default_model } });
  }
