...
class z2ui5_cl_core_srv_model {
  ...
  main_json_to_attri({ view, model } = {}) {
    ...
    for (const lr_attri of this.mt_attri) {
      ...
      try {
        ...
        throw new z2ui5_cx_a2ui5_error({ val: `JSON_PARSING_ERROR: ${x.get_text()}` });
      }
    }
  }
  ...
  main_json_stringify() {
    ...
    try {
      ...
      for (const lr_attri of this.mt_attri) {
        sy_tabix++;
        if (!(lr_attri.bind_type !== `` && lr_attri.type_kind !== z2ui5_cl_a2ui5_context.cv_typedescr_typekind_dref && lr_attri.type_kind !== z2ui5_cl_a2ui5_context.cv_typedescr_typekind_oref)) continue;
  ...
  main_attri_db_load() {
    ...
    for (const lr_attri of this.mt_attri) {
      ...
      switch (lr_attri.type_kind) {
        case z2ui5_cl_a2ui5_context.cv_typedescr_typekind_table:
          this.main_attri_db_load_table({ ir_attri: lr_attri });
          break;
        case z2ui5_cl_a2ui5_context.cv_typedescr_typekind_dref:
    ...
    for (const lr_attri of this.mt_attri) {
      ...
      try {
        ...
        if (lr_attri.srtti_data) {
          // TODO(abap2js): ASSIGN lr_ref->* TO FIELD-SYMBOL(<val>).
          fs_val = z2ui5_cl_a2ui5_context.xml_srtti_parse(lr_attri.srtti_data);
  ...
  main_attri_db_load_dref({ ir_attri, ir_child_idx } = {}) {
    ...
    ir_attri.o_typedescr = z2ui5_cl_a2ui5_context.rtti_get_typedescr_by_data_ref(fs_parent_ref);
    sy_tabix = 0;
  ...
  main_attri_db_save_srtti() {
    ...
    for (const lr_attri of this.mt_attri) {
      sy_tabix++;
      if (!(!lr_attri.name_ref && lr_attri.type_kind === z2ui5_cl_a2ui5_context.cv_typedescr_typekind_dref)) continue;
      ...
      lo_descr = z2ui5_cl_a2ui5_context.rtti_get_typedescr_by_data(fs_val1);
      switch (lo_descr.type_kind) {
        case z2ui5_cl_a2ui5_context.cv_typedescr_typekind_table:
          ...
          for (const lr_attri_child of this.mt_attri) {
            sy_tabix++;
            if (!(!lr_attri_child.name_ref && lr_attri_child.type_kind === z2ui5_cl_a2ui5_context.cv_typedescr_typekind_table && lr_attri_child.name_parent === lr_attri.name)) continue;
            lv_name6 = `MO_APP->${lr_attri_child.name}`;
            // TODO(abap2js): ASSIGN (lv_name6) TO FIELD-SYMBOL(<val_ref>).
            if (sy_subrc !== 0) {
              continue;
            }
            lr_attri.srtti_data = z2ui5_cl_a2ui5_context.xml_srtti_stringify(fs_val_ref);
            fs_val_ref = null;
          ...
          break;
        case z2ui5_cl_a2ui5_context.cv_typedescr_typekind_struct1:
        case z2ui5_cl_a2ui5_context.cv_typedescr_typekind_struct2:
          lr_attri.srtti_data = z2ui5_cl_a2ui5_context.xml_srtti_stringify(fs_val1);
          break;
      }
    }
    sy_tabix = 0;
    for (const lr_attri2 of this.mt_attri) {
      sy_tabix++;
      if (!(lr_attri2.type_kind === z2ui5_cl_a2ui5_context.cv_typedescr_typekind_dref)) continue;
  ...
  main_attri_search({ val } = {}) {
    ...
    throw new z2ui5_cx_a2ui5_error({ val: `BINDING_ERROR - No class attribute for binding found - Please check if the bound values are public attributes of your class` });
    return result;
  }
  ...
  attri_get_val_ref({ iv_path } = {}) {
    ...
    if (!(fs_attri != null)) {
      throw new z2ui5_cx_a2ui5_error({ val: `ATTRI_GET_VAL_REF_ERROR` });
    }
  ...
  attri_search({ val } = {}) {
    ...
    let lv_name_val;
    let lr_ref;
    const lo_datadescr = z2ui5_cl_a2ui5_context.rtti_get_typedescr_by_data_ref(val);
    if (lo_datadescr.type_kind === z2ui5_cl_a2ui5_context.cv_typedescr_typekind_dref || lo_datadescr.type_kind === z2ui5_cl_a2ui5_context.cv_typedescr_typekind_oref) {
      throw new z2ui5_cx_a2ui5_error({ val: `NO DATA REFERENCES FOR BINDING ALLOWED: DEREFERENCE YOUR DATA FIRST` });
    }
    sy_tabix = 0;
    for (const lr_attri of this.mt_attri) {
      sy_tabix++;
  ...
  attri_create_new({ name } = {}) {
    let result = null;
    const lo_descr = z2ui5_cl_a2ui5_context.rtti_get_typedescr_by_data_ref(this.attri_get_val_ref({ iv_path: name }));
  ...
  diss_dref({ ir_attri } = {}) {
    ...
    const lr_ref = z2ui5_cl_a2ui5_context.unassign_data(lr_ref_tmp);
    if (!lr_ref) {
      return result;
    }
    const ls_attri2 = {};
    ...
    switch (ls_attri2.o_typedescr.kind) {
      case z2ui5_cl_a2ui5_context.cv_typedescr_kind_struct:
  ...
  diss_oref({ ir_attri } = {}) {
    ...
    const lr_ref = z2ui5_cl_a2ui5_context.unassign_object(lr_val);
    const lt_attri = z2ui5_cl_a2ui5_context.rtti_get_t_attri_by_oref(lr_ref);
    const lv_prefix = (ir_attri.name ? `${ir_attri.name}->` : null);
    sy_tabix = 0;
    for (const lr_attri of lt_attri) {
      sy_tabix++;
      if (!(lr_attri.visibility === z2ui5_cl_a2ui5_context.cv_objectdescr_public && !(lr_attri.is_interface === true || lr_attri.is_interface === `X`) && !(lr_attri.is_class === true || lr_attri.is_class === `X`) && !(lr_attri.is_constant === true || lr_attri.is_constant === `X`))) continue;
      try {
      ...
      switch (lr_attri.type_kind) {
        case z2ui5_cl_a2ui5_context.cv_typedescr_typekind_table:
          ...
          for (const lr_attri_ref of this.mt_attri) {
            sy_tabix++;
            if (!((lr_attri_ref.check_dissolved === true || lr_attri_ref.check_dissolved === `X`) && lr_attri_ref.name !== lr_attri.name && !lr_attri_ref.name_ref && lr_attri_ref.type_kind === z2ui5_cl_a2ui5_context.cv_typedescr_typekind_table)) continue;
            try {
        ...
        case z2ui5_cl_a2ui5_context.cv_typedescr_typekind_dref:
          ...
          for (const lr_attri_ref of this.mt_attri) {
            sy_tabix++;
            if (!((lr_attri_ref.check_dissolved === true || lr_attri_ref.check_dissolved === `X`) && lr_attri_ref.name !== lr_attri.name && !lr_attri_ref.name_ref && (lr_attri_ref.type_kind === z2ui5_cl_a2ui5_context.cv_typedescr_typekind_struct1 || lr_attri_ref.type_kind === z2ui5_cl_a2ui5_context.cv_typedescr_typekind_struct2))) continue;
            try {
              lr_attri_ref_ref = this.attri_get_val_ref({ iv_path: lr_attri_ref.name });
            } catch (error) {
              continue;
    ...
    for (const lr_attri of this.mt_attri) {
      ...
      switch (lr_attri.o_typedescr.kind) {
        case z2ui5_cl_a2ui5_context.cv_typedescr_kind_struct:
          ...
          break;
        case z2ui5_cl_a2ui5_context.cv_typedescr_kind_ref:
          switch (lr_attri.o_typedescr.type_kind) {
            case z2ui5_cl_a2ui5_context.cv_typedescr_typekind_oref:
  ...
  delta_apply_nodes({ io_delta, ct_tab } = {}) {
    ...
    let fs_sub_tab = null;
    let _fs$fs_sub_tab = null;
    let lv_tabix;
    let lo_row_d;
    let lt_fld;
    let lv_fld_path;
    let lo_sub;
    const lt_idx = io_delta.members(`/`);
    sy_tabix = 0;
    for (const lv_idx_str of lt_idx) {
      ...
      for (const lv_fld of lt_fld) {
        ...
        lv_fld_path = `/${lv_fld}`;
        switch (lo_row_d.get_node_type(lv_fld_path)) {
          ...
          case z2ui5_if_ajson_types.node_type.object:
            lo_sub = lo_row_d.slice(lv_fld_path);
            if ((lo_sub.exists(`/__delta`) === true || lo_sub.exists(`/__delta`) === `X`)) {
              let lr_sub = null;
          ...
          case z2ui5_if_ajson_types.node_type.array:
            // TODO(abap2js): lo_row_d->slice( lv_fld_path )->to_abap( EXPORTING iv_corresponding = abap_true IMPORTING ev_container = <comp> ).
            break;
          default:
