...
const { lcl_mapping_fields, lcl_rename, lcl_mapping_to_upper, lcl_mapping_to_lower, lcl_mapping_camel, lcl_compound_mapper, lcl_to_snake, lcl_to_camel } = require("abap2UI5/z2ui5_cl_ajson_mapping").__locals;
...
class ltcl_test_mappers {
  ...
  to_abap() {
    ...
    const _out0 = { ev_container: ls_result };
    lo_ajson.to_abap(_out0);
    if ("ev_container" in _out0) ls_result = _out0.ev_container;
  ...
  to_json_nested_struc() {
    let lo_ajson = null;
    let li_mapping = null;
    let ls_result = { field_data: ``, struc_data: { field_more: `` } };
    li_mapping = z2ui5_cl_ajson_mapping.create_camel_case({ iv_first_json_upper: false });
    ls_result.field_data = `field_value`;
  ...
  to_json_nested_table() {
    ...
    ls_result.field_data = `field_value`;
    lv_value = `field_more`;
    ls_result.struc_data.field_more.push(z2ui5_cl_util.abap_copy(lv_value));
    lo_ajson = z2ui5_cl_ajson.create_empty({ ii_custom_mapping: li_mapping });
    lo_ajson.set({ iv_path: `/`, iv_val: ls_result });
    cl_abap_unit_assert.assert_equals({ act: lo_ajson.stringify(), exp: `{"fieldData":"field_value","strucData":{"fieldMore":["field_more"]}}` });
  }
...
class ltcl_fields {
  to_abap() {
    let lo_ajson = null;
    let li_mapping = null;
    let lt_mapping_fields = [];
    let ls_mapping_field = { abap: ``, json: `` };
    let ls_result = { abap_field: ``, field: `` };
    ls_mapping_field = { abap: ``, json: `` };
    ls_mapping_field.abap = `ABAP_FIELD`;
    ls_mapping_field.json = `json.field`;
    lt_mapping_fields.push(z2ui5_cl_util.abap_copy(ls_mapping_field));
