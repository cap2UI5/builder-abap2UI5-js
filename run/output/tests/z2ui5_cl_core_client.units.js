...
class ltcl_test_client {
  ...
  test_follow_up_action_nav() {
    ...
    cl_abap_unit_assert.assert_equals({ exp: 2, act: this.mo_action.ms_next.s_set.s_follow_up_action.custom_js.length });
    cl_abap_unit_assert.assert_equals({ exp: `.eF('CONTROL_BY_ID', 'myContainer', 'MAIN', 'to', 'myPage')`, act: this.mo_action.ms_next.s_set.s_follow_up_action.custom_js[(1) - 1] });
  ...
  test_set_nav_back() {
    let temp27 = null;
    let li_client = null;
    temp27 = z2ui5_cl_util.abap_cast(this.mo_client);
    li_client = temp27;
