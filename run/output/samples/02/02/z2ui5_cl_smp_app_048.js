...
class z2ui5_cl_smp_app_048 extends z2ui5_if_app {
  ...
  async main(client) {
    ...
    page.message_strip({ text: `A List of generic StandardListItems showing highlight bars, colored infoState and ` + `wrap-character limits; the detail button and selection changes raise backend events with message boxes.`, type: `Information`, showicon: true, class: `sapUiSmallMargin` });
    page.list({ headertext: `List Output`, items: client._bind(this.t_tab), mode: `SingleSelectMaster`, selectionchange: client._event(`SELCHANGE`) })
