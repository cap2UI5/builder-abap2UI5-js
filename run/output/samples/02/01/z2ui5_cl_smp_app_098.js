...
class z2ui5_cl_smp_app_098 extends z2ui5_if_app {
  ...
  view_display_detail() {
    ...
    const tab = page.ui_table({ rows: this.client._bind(this.t_tab2), editable: false, alternaterowcolors: true, rowactioncount: `1`, fixedcolumncount: `1`, selectionmode: `None`, sort: this.client._event(`SORT`), filter: this.client._event(`FILTER`), customfilter: this.client._event(`CUSTOMFILTER`) });
  ...
  view_display_master() {
    ...
    page.message_strip({ text: `A three-column FlexibleColumnLayout: select a row to open the detail column, then ` + `navigate on to open a third, deeply nested column.`, type: `Information`, showicon: true, class: `sapUiSmallMargin` });
