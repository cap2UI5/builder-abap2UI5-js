...
class z2ui5_cl_smp_app_499 extends z2ui5_if_app {
  ...
  view_display() {
    ...
    detail.tag(`MessageStrip`)
      .a({ n: `text`, v: `This page is #/detail. Reload the browser or share the URL - it lands here. ` + `The back arrow is cs_event-hash_back with '/' as fallback: normally a real ` + `window.history.go(-1), and on a cold deep link a replace to the start page. ` + `Edit the variant in the URL to something unknown (#/detail/z) and watch it repaired.` })
      .a({ n: `type`, v: `Success` })
      .a({ n: `showIcon`, b: true })
      .a({ n: `class`, v: `sapUiSmallMargin` });
    detail.tag(`Button`)
  ...
  on_event() {
    switch (this.client.get_event()) {
      ...
      case `GO_MAIN`:
        ...
        this.client.follow_up_action(this.client.cs_event.control_by_id, [`nav`, `to`, `page-main`]);
        this.client.follow_up_action(this.client.cs_event.hash_set, [`/`]);
        break;
      case `VARIANT`:
      ...
      case `HASH_CHANGED`:
        this.hash_apply();
        if ((this.check_detail === true || this.check_detail === `X`) && this.client.get().S_CONFIG.HASH !== `/detail/${this.variant}`) {
          this.client.follow_up_action(this.client.cs_event.hash_replace, [`/detail/${this.variant}`]);
        }
