...
class z2ui5_cl_smp_app_498 extends z2ui5_if_app {
  ...
  view_display() {
    ...
    form.tag(`Label`).a({ n: `text`, v: `keep the state id in the URL` });
    form.tag(`Switch`)
      .a({ n: `state`, v: this.client._bind(this.state_in_url) })
      .a({ n: `change`, v: this.client._event(`TOGGLE_STATE`) });
    page.tag(`MessageStrip`)
  ...
  on_event() {
    switch (this.client.get_event()) {
      case `POST`:
        ...
        break;
      case `TOGGLE_STATE`:
        if ((this.state_in_url === true || this.state_in_url === `X`)) {
          this.client.follow_up_action(z2ui5_if_client.cs_event.app_state_set_active);
          this.client.message_toast_display(`the URL names the current state again`);
        } else {
          this.client.follow_up_action(z2ui5_if_client.cs_event.app_state_set_active, [` `]);
          this.client.message_toast_display(`switched off - the URL stops following the state`);
        }
        break;
      case `SHARE`:
