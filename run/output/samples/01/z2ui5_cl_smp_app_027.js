...
` + `  path:"` + bind_input32 + `" }` }).tag(`Input`).a({ n: `enabled`, b: false }).a({ n: `value`, v: `{= Math.max($${this.client._bind(this.input31)}, $${this.client._bind(this.input32)}) }` }).tag(`Label`).a({ n: `text`, v: `only enabled when the quantity equals 500` }).tag(`Input`).a({ n: `value`, v: `{ type : "sap.ui.model.type.Integer",` + `  path:"` + bind_quantity + `" }` }).tag(`Input`).a({ n: `enabled`, v: `{= 500===$${this.client._bind(this.quantity)} }` }).a({ n: `value`, t: this.product }).tag(`Label`).a({ n: `text`, v: `RegExp Set to enabled if the input contains VIP, ignoring the case.` }).tag(`Input`).a({ n: `value`, v: this.client._bind(this.input41) }).tag(`Button`).a({ n: `text`, v: `VIP` }).a({ n: `enabled`, v: `{= RegExp('vip', 'i')
