...
class z2ui5_cl_smp_app_282 extends z2ui5_if_app {
  client = null;

  view_display({ client } = {}) {
    const page_01 = z2ui5_cl_xml_view.factory()
      .shell()
      .page({ title: `abap2UI5 - Sample: InvisibleText`, navbuttonpress: client._event_nav_app_leave(), shownavbutton: client.check_app_prev_stack() });
    page_01.header_content()
      .button({ id: `button_hint_id`, icon: `sap-icon://hint`, tooltip: `Sample information`, press: client._event(`CLICK_HINT_ICON`) });
    page_01.header_content()
      .link({ text: `UI5 Demo Kit`, target: `_blank`, href: `https://sapui5.hana.ondemand.com/sdk/#/entity/sap.ui.core.InvisibleText/sample/sap.ui.core.sample.InvisibleText` });
    const page_02 = page_01.page({ title: `Page`, class: `sapUiContentPadding` })
      ...
      .title(`Title`)
      .toolbar_spacer()
      .button({ icon: `sap-icon://edit`, press: client._event(`onPress`, [`\${$source>/id}`]), arialabelledby: `editButtonLabel` })
      .get_parent()
      .get_parent()
      .sub_header()
      .toolbar()
      ...
      .button({ icon: `sap-icon://action`, press: client._event(`onPress`, [`\${$source>/id}`]), arialabelledby: `actionButtonLabel` })
      .toolbar_spacer()
      .get_parent()
      .get_parent()
      .content()
      ...
      .button({ text: `Default`, press: client._event(`onPress`, [`\${$source>/id}`]) })
      .button({ icon: `sap-icon://action`, press: client._event(`onPress`, [`\${$source>/id}`]) })
      .get_parent()
      .get_parent();
  ...
  popover_display({ id } = {}) {
    ...
    view.quick_view({ placement: `Bottom`, width: `auto` })
      .quick_view_page({ pageid: `sampleInformationId`, header: `Sample information`, description: `Many controls provide the associations ariaLabelledBy and ariaDescribedBy for accessibility purposes. ` + `The InvisibleText control can be used by application to provide hidden texts on the UI which can be referenced via these associations.` });
...
module.exports = z2ui5_cl_smp_app_282;
