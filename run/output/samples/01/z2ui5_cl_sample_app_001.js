...
class z2ui5_cl_sample_app_001 extends z2ui5_if_app {
  client = null;
  s_scroll = { id: ``, x: 0, y: 0 };
  ...
  scroll_restore() {
    ...
    this.client.action.gen({ val: z2ui5_if_client.cs_event.scroll_to, t_arg: [this.s_scroll.id, `${this.s_scroll.y}`, `${this.s_scroll.x}`] });
  }
  ...
  view_display() {
    ...
    if ((this.class_exists({ name: `Z2UI5_CL_SAMPLE_APP_000` }) === true || this.class_exists({ name: `Z2UI5_CL_SAMPLE_APP_000` }) === `X`)) {
      url_restricted = `${this.client.get()
        .S_CONFIG.ORIGIN}${this.client.get()
        .S_CONFIG.PATHNAME}?app_start=z2ui5_cl_sample_app_000`;
      page.header_content()
    ...
    if ((this.class_exists({ name: `Z2UI5_CL_DEMO_APP_000` }) === true || this.class_exists({ name: `Z2UI5_CL_DEMO_APP_000` }) === `X`)) {
      url = `${this.client.get()
        .S_CONFIG.ORIGIN}${this.client.get()
        .S_CONFIG.PATHNAME}?app_start=z2ui5_cl_demo_app_000`;
      page.message_strip({ type: `Warning`, showicon: true, enableformattedtext: true, class: `sapUiSmallMarginBottom`, text: `This overview is still under construction. Click <a href="${url}" target="_blank">here</a> to open the classic overview.` });
    }
    let prev_group = ``;
    let prev_base = ``;
    sy_tabix = 0;
    for (const tile of t_catalog) {
      ...
      if (tile.group !== prev_group) {
        ...
        prev_group = z2ui5_cl_util.abap_copy(tile.group);
      } else if (base !== prev_base) {
        new_block = true;
      }
      prev_base = z2ui5_cl_util.abap_copy(base);
  ...
  block_widths({ t_catalog } = {}) {
    ...
    for (const tile of t_catalog) {
      ...
      if (width > fs_block.width) {
        fs_block.width = z2ui5_cl_util.abap_copy(width);
      }
    }
    return result;
  }
  ...
  header_base({ header } = {}) {
    let result = ``;
    result = z2ui5_cl_util.abap_copy(header);
...
module.exports = z2ui5_cl_sample_app_001;
