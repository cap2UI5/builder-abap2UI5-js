...
class z2ui5_cl_demo_app_532 extends z2ui5_if_app {
  view_display({ client } = {}) {
    const page = z2ui5_cl_xml_view.factory()
      .shell()
      .page({ title: `abap2UI5 - Sample: Object Page AnchorBar with numbers`, navbuttonpress: client._event_nav_app_leave(), shownavbutton: client.check_app_prev_stack() });
    page.header_content()
      .link({ text: `UI5 Demo Kit`, target: `_blank`, href: `https://sapui5.hana.ondemand.com/sdk/#/entity/sap.uxap.ObjectPageLayout/sample/sap.uxap.sample.AnchorBarWithNumbers` });
    const object_page_layout = page.object_page_layout({ subsectionlayout: `TitleOnLeft`, showtitleinheadercontent: true, uppercaseanchorbar: false });
    ...
    header_title.snapped_heading()
      ...
      .icon({ src: `sap-icon://picture`, class: `sapUiTinyMarginEnd` })
    ...
    flex_box.icon({ src: `sap-icon://picture`, size: `2.5rem`, class: `sapUiSmallMarginEnd` });
  ...
  render_goals_section({ sections } = {}) {
    sections.object_page_section({ titleuppercase: false, title: `2014 Goals Plan` })
      .sub_sections()
      .object_page_sub_section({ title: `Goal summary`, titleuppercase: false })
      .blocks()
  ...
  render_personal_section({ sections } = {}) {
    const sub_sections = sections.object_page_section({ titleuppercase: false, title: `Personal (2)` }).sub_sections();
    const connect_blocks = sub_sections.object_page_sub_section({ title: `Connect`, titleuppercase: false });
    ...
    const payment = sub_sections.object_page_sub_section({ title: `Payment information`, titleuppercase: false });
    payment.blocks()
      .simple_form({ editable: false, layout: `ColumnLayout` })
  ...
  render_employment_section({ sections } = {}) {
    let sy_tabix = 0;
    const sub_sections = sections.object_page_section({ titleuppercase: false, title: `Employment (3)` })
      .sub_sections();
    const job_info_blocks = sub_sections.object_page_sub_section({ title: `Job information`, titleuppercase: false })
      .blocks();
    ...
    const details = sub_sections.object_page_sub_section({ title: `Employee Details`, titleuppercase: false });
    details.blocks()
    ...
    const job_grid = sub_sections.object_page_sub_section({ title: `Job Relationship`, titleuppercase: false })
      .blocks()
...
module.exports = z2ui5_cl_demo_app_532;
