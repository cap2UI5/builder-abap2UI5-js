...
class z2ui5_cl_demo_app_456 extends z2ui5_if_app {
  ...
  view_display({ client } = {}) {
    const page = z2ui5_cl_xml_view.factory()
      .shell()
      .page({ title: `abap2UI5 - Sample: MultiInput Data Binding`, navbuttonpress: client._event_nav_app_leave(), shownavbutton: client.check_app_prev_stack() });
    page.header_content()
      .link({ text: `UI5 Demo Kit`, target: `_blank`, href: `https://sapui5.hana.ondemand.com/sdk/#/entity/sap.m.MultiInput/sample/sap.m.sample.MultiInputDatabinding` });
    const layout = page.vertical_layout({ class: `sapUiContentPadding`, width: `100%` });
    layout.label({ text: `Token data binding`, labelfor: `multiinput1` });
    layout.multi_input({ id: `multiinput1`, showvaluehelp: false, showsuggestion: false, tokens: client._bind(this.t_products) })
      .tokens()
      .token({ key: `{PRODUCT_ID}`, text: `{NAME}` });
