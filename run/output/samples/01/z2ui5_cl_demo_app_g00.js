...
class z2ui5_cl_demo_app_g00 extends z2ui5_if_app {
  client = null;
  s_scroll = { id: ``, x: 0, y: 0 };
  ...
  on_event() {
    let classname;
    try {
      classname = this.client.get().EVENT.toUpperCase();
      let li_app = null;
      li_app = (() => { const _n = String(classname); const _c = z2ui5_cl_util.rtti_get_class(_n.toLowerCase()); if (!_c) throw new Error(`CREATE OBJECT: class ${_n} not found`); return new _c(); })();
  ...
  view_display() {
    ...
    let width;
    let row;
    const t_catalog = this.get_catalog();
    const t_blocks = this.block_widths({ t_catalog: t_catalog });
    const view = z2ui5_cl_xml_view.factory();
    const page = view.shell()
      .page({ id: `page`, title: `abap2UI5 - Samples`, navbuttonpress: this.client._event_nav_app_leave(), shownavbutton: this.client.check_app_prev_stack() });
    if ((this.class_exists({ name: `Z2UI5_CL_SAMPLE_APP_G01` }) === true || this.class_exists({ name: `Z2UI5_CL_SAMPLE_APP_G01` }) === `X`)) {
      url_restricted = `${this.client.get()
        .S_CONFIG.ORIGIN}${this.client.get()
        .S_CONFIG.PATHNAME}?app_start=z2ui5_cl_sample_app_g01`;
      page.header_content()
        .button({ text: `Extended Samples`, icon: `sap-icon://action`, press: this.client._event_client(this.client.cs_event.open_new_tab, [url_restricted]) });
    }
    let prev_group = ``;
    let prev_base = ``;
    sy_tabix = 0;
    for (const tile of t_catalog) {
      ...
      row = page.hbox({ alignitems: `Center`, wrap: `Wrap`, class: ((new_block === true || new_block === `X`) ? `sapUiTinyMarginBegin sapUiSmallMarginTop` : `sapUiTinyMarginBegin`) });
      if (!tile.sub) {
        row.link({ text: tile.header, width, press: this.client._event(tile.app) });
      } else {
        row.link({ text: tile.header, width, press: this.client._event(tile.app) }).text(tile.sub);
      }
    }
  ...
  header_base({ header } = {}) {
    ...
    if (n > 1 && (words[(n) - 1]) && [...String(words[(n) - 1])].every(($c) => String(`IVXLCDM`).includes($c))) {
...
module.exports = z2ui5_cl_demo_app_g00;
